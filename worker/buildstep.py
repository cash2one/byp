...
def genSvnLockActions(product, action, value):
    ...
    for item in commands:
        ...
        command = 'svn list --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 -R ' + item + ' > ' + infoFile
        ...
        for f in files:
            ...
            if f[-1] == '/' or f[-1] == '\\':
                continue
            if action == 'lock':
                cmd_details.append('svn lock --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --force ' + item + '/' + f.decode(ecode).encode('utf-8'))
            elif action == 'unlock':
                cmd_details.append('svn unlock --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --force ' + item + '/' + f.decode(ecode).encode('utf-8'))
    return cmd_details
        ...
        try:
            ...
            for node in root.childNodes:
                ...
                if svnAction == 'checkout':
                    command = "svn " + svnAction + " --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + svnDir + " " + conf.sln_root + dir
                elif svnAction == 'update':
                    commands.append("svn cleanup --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 " + conf.sln_root + dir)
                    command = "svn " + svnAction + " --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.sln_root + dir
            ...
            logging.error(e)
    if svnAction == 'update':
        commands.append("svn cleanup --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 " + conf.sln_root + "basic")
        commands.append("svn update --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.sln_root + "basic")
        commands.append("svn cleanup --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 " + conf.sln_root + "stable_proj")
        commands.append("svn update --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.sln_root + "stable_proj")
    elif svnAction == 'checkout' and value != 5:
        if product == 'bdm':
            commands.append("svn checkout --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "basic_proj" + codeDir + " " + conf.sln_root + "basic")
        elif product == 'bdkv':
            commands.append("svn checkout --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "avbasic_proj" + codeDir + " " + conf.sln_root + "basic")
        commands.append("svn checkout --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "stable_proj" + codeDir + " " + conf.sln_root + "stable_proj")
    commands = list(set(commands))
    return commands
...
def ExpendMarkupValue(product, str, code_revision, codeDir):
    ...
    if code_revision != '' or (not os.path.exists(local_version_file)):
        ...
        os.system(command)
        command = 'svn checkout --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision ' + revision + ' ' + remote_version_folder + ' ' + local_version_folder
        os.system(command)
...
def getMarkupCodeCommands(product, value):
    ...
    if markupType == '' or markupValue == '' or markupType == 'none':
        ...
        for item in slns:
            ...
            try:
                ...
                if not svnDir:
                    svnDir = dir + codeDir
                if actType[0] == '+':
                    command = "svn copy --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + svnDir + " " + conf.svn_url + dir + markupType + markupValue + ' -m "' + msg + '"'
                elif actType[0] == '-':
        ...
        if actType[0] == '+' and (not xmarkup):
            if product == 'bdm':
                commands.append("svn copy --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "basic_proj" + codeDir + " " + conf.svn_url + "basic_proj" + markupType + markupValue + ' -m "' + msg + '"')
            elif product == 'bdkv':
                commands.append("svn copy --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "avbasic_proj" + codeDir + " " + conf.svn_url + "avbasic_proj" + markupType + markupValue + ' -m "' + msg + '"')
            commands.append("svn copy --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "stable_proj" + codeDir + " " + conf.svn_url + "stable_proj" + markupType + markupValue + ' -m "' + msg + '"')
            #commands.append("svn copy --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision " + revision + " " + conf.svn_url + "common_stage_proj" + codeDir + " " + conf.svn_url + "common_stage_proj" + markupType + markupValue + ' -m "' + msg + '"')
...
def genSymbols(product):
    ...
    if product == 'bdm':
        ...
        commands.append(makeBinplace('bdm', 'basic\\Output\\BinRelease\\plugins\\bdmsusplugins\\*.dll', 'Release'))
        ...
        commands.append(makeBinplace('bdm', 'basic\\Output\\BinDebug\\plugins\\bdmsusplugins\\*.dll', 'Debug'))
...
def genPrebuildActions(product, value):
    ...
    commands.append('svn update --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 --revision HEAD ' + conf.sln_root + 'basic')
    return commands
...
class PreBuild(BuildStep):
    ...
    def act(self):
        if self.value == 0:
            ...
            for item in commands:
                self.report('wk-build-log', item.replace('bCRjzYKzk272', 'XXXXXX'))
...
class Svn(BuildStep):
    ...
    def act(self):
        if self.value == 0:
            ...
            command = 'svn info --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 ' + conf.svn_url + ' > ' + conf.svn_remote_info_file
            os.system(command)
...
class Install(BuildStep):
    ...
    def act(self):
        if self.value == 0:
            ...
            if bIgnoreFault:
                command = 'svn update --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 ' + conf.sln_root + 'basic --accept mine-full'
                os.system(command.encode(sys.getfilesystemencoding()))
            #install
            (bInstall, bInstallMini, bInstallFull, bInstallUpdate, bInstallSilence, bInstallDefense, bInstallCombine) = getInstallOptions()
            if bInstall:
...
class Commit(BuildStep):
    ...
    def act(self):
        if self.value == 0:
            ...
            command = 'svn commit --non-interactive --no-auth-cache --username buildbot --password bCRjzYKzk272 ' + conf.sln_root + 'basic -m "%s" --no-unlock' % msg
            self.report('wk-build-log', command.replace('bCRjzYKzk272', 'XXXXXX'))
            os.system(command.encode(sys.getfilesystemencoding()))
            self.update_step(10)
        BuildStep.act(self)
